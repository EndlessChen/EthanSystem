...
class UserServiceImplTest {
    ...
    public void testRegister() {
        String userAccount = "EthanChen", userPassword = "12345678", verifyPassword = "12345678";
        String inviteCode = UUID.randomUUID().toString();
        // 1. 输入信息存在空值时无法注册
        verifyPassword = "";
        Long id = userService.userRegister(userAccount, userPassword, verifyPassword, inviteCode);
        Assertions.assertEquals(-1L, id);
        // 2. 输入信息不合规时无法注册
        userPassword = "12345";
        verifyPassword = "12345";
        id = userService.userRegister(userAccount, userPassword, verifyPassword, inviteCode);
        Assertions.assertEquals(-1L, id);
        // 3. 用户账户存在时无法注册
